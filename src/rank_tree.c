...
#define TREE_PAGE_SIZE 4096U
#define TREE_FANOUT ((uint16_t)(TREE_PAGE_SIZE / sizeof(void*)))

#if UINTPTR_MAX > UINT32_MAX // 64-bit
#  define INDEX_BITS 9U
#  define INDEX_MASK 0x1FFU
#  define MAX_HEIGHT 7U
#  define MAX_LEVELS 8U
#else // 32-bit
#  define INDEX_BITS 10U
#  define INDEX_MASK 0x3FFU
#  define MAX_HEIGHT 3U
#  define MAX_LEVELS 4U
#endif
...
struct ZixRankTreeImpl {
  ...
  unsigned         height;    ///< Height where 0 has one node (the root)
};

union ZixRankTreeNodeImpl {
  uintptr_t        vals[TREE_FANOUT];     ///< Elements (leaf)
  ZixRankTreeNode* children[TREE_FANOUT]; ///< Children (internal)
};

#if ((defined(__STDC_VERSION__) && __STDC_VERSION__ >= 201112L) || \
     (defined(__cplusplus) && __cplusplus >= 201103L))
static_assert(sizeof(ZixRankTree) <= TREE_PAGE_SIZE, "");
static_assert(sizeof(ZixRankTreeNode) == TREE_PAGE_SIZE, "");
#endif
...
zix_rank_tree_parse_rank(const size_t rank)
{
  ...
  for (unsigned i = 0U; i < MAX_LEVELS; ++i) {
    path.indices[MAX_HEIGHT - i] = remaining & INDEX_MASK;
    remaining                    = remaining >> INDEX_BITS;
  }
...
zix_rank_tree_node_new(ZixAllocator* const allocator)
{
  ZixRankTreeNode* const node = (ZixRankTreeNode*)zix_aligned_alloc(
    allocator, TREE_PAGE_SIZE, TREE_PAGE_SIZE);

  if (node) {
    memset(node, 0, TREE_PAGE_SIZE);
  }
...
#if !((defined(__STDC_VERSION__) && __STDC_VERSION__ >= 201112L) || \
      (defined(__cplusplus) && __cplusplus >= 201103L))
  assert(sizeof(ZixRankTree) <= TREE_PAGE_SIZE);
  assert(sizeof(ZixRankTreeNode) == TREE_PAGE_SIZE);
#endif
                            ...
                            void* const                  destroy_user_data)
{
  if (height) {
    ...
    for (ChildIndex i = 0U; i < TREE_FANOUT; ++i) {
      this_path.indices[MAX_HEIGHT - height] = i;
    ...
    unsigned n_vals = last_path.indices[MAX_HEIGHT] + 1U;
    for (unsigned i = 1U; i <= t->height; ++i) {
      if (this_path.indices[MAX_HEIGHT - i] <
          last_path.indices[MAX_HEIGHT - i]) {
        n_vals = TREE_FANOUT;
        break;
      }
    }
...
{
  assert(t);
  assert(t->height < UINT8_MAX);
  return (uint8_t)t->height;
}
...
zix_rank_tree_at(const ZixRankTree* const t, const size_t rank)
{
  ...
  const unsigned        root_level = MAX_HEIGHT - t->height;
  ZixRankTreeNode*      node       = t->root;
  for (unsigned i = root_level; i < MAX_HEIGHT; ++i) {
    assert(node->children[path.indices[i]]);
    node = node->children[path.indices[i]];
  }

  return node->vals[path.indices[MAX_HEIGHT]];
}
...
zix_rank_tree_push(ZixRankTree* const t, const uintptr_t e)
{
  ...
  const unsigned        height_bits = n_levels * INDEX_BITS;
  ...
  node->vals[path.indices[MAX_HEIGHT]] = e;
...
zix_rank_tree_pop(ZixRankTree* const t)
{
  ...
  // Decrease size, and handle now-empty case by clearing entirely
  const size_t rank = --t->size;
  if (!rank) {
    zix_aligned_free(t->allocator, t->root);
    t->root = NULL;
  ...
  const ZixRankTreePath path                = zix_rank_tree_parse_rank(rank);
  ZixRankTreeNode*      parents[MAX_LEVELS] = {NULL};
  ZixRankTreeNode*      node                = t->root;
  const unsigned        root_level          = MAX_HEIGHT - t->height;
  for (unsigned i = root_level; i < MAX_HEIGHT; ++i) {
    parents[i] = node;
  ...
  node->vals[path.indices[MAX_HEIGHT]] = 0U;

  if (!path.indices[MAX_HEIGHT]) {
    // Leaf is now empty, walk up deleting now-empty nodes as necessary
    for (int i = MAX_HEIGHT - 1U; i >= (int)root_level; --i) {
    ...
    assert(t->height); // Now-empty root is handled as a special case above
    if (!t->root->children[1U]) {
