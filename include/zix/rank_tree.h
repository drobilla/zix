...
/**
   ZIX_RANK_TREE_ELEMENT_TYPE can be defined to make the API more type-safe.

   Elements are opaque to the tree internals, but must have the same size and
   alignment as uintptr_t.
*/
#ifdef ZIX_RANK_TREE_ELEMENT_TYPE
typedef ZIX_RANK_TREE_ELEMENT_TYPE ZixRankTreeElement;
#else
typedef uintptr_t ZixRankTreeElement;
#endif

#if ((defined(__STDC_VERSION__) && __STDC_VERSION__ >= 202311L) || \
     (defined(__cplusplus) && __cplusplus >= 201103L))
static_assert(sizeof(ZixRankTreeElement) == sizeof(uintptr_t),
              "ZixRankTreeElement must be pointer-sized");
static_assert(alignof(ZixRankTreeElement) == alignof(uintptr_t),
              "ZixRankTreeElement must be pointer-aligned");
#elif defined(__STDC_VERSION__) && __STDC_VERSION__ >= 201112L
_Static_assert(sizeof(ZixRankTreeElement) == sizeof(uintptr_t),
               "ZixRankTreeElement must be pointer-sized");
_Static_assert(_Alignof(ZixRankTreeElement) == _Alignof(uintptr_t),
               "ZixRankTreeElement must be pointer-aligned");
#endif
...
/**
   ...
   entirely with either child pointers or elements with zero overhead).

   The tree stores opaque pointer-sized elements, and is structured so that
...
typedef void (*ZixRankTreeDestroyFunc)(ZixRankTreeElement    element,
...
*/
ZIX_PURE_API ZixRankTreeElement
...
/**
   ...
   @return #ZIX_STATUS_SUCCESS, or #ZIX_STATUS_NO_MEM if allocation fails.
*/
ZIX_API ZixStatus
zix_rank_tree_push(ZixRankTree* ZIX_NONNULL t, ZixRankTreeElement e);

/**
   Remove the last element from `t`.

   @param t Tree to pop the last element from.
